...
export default function Loginonly() {
  ...
  /* ✅ ALL-IN-ONE FACE ID (LOGIN + AUTO REGISTER) */
    ...
    try {
      ...
      if (options.error) {
        ...
        if (regResult.verified) {
            localStorage.setItem("agasthyaUser", phone); // ✅ REMEMBER USER
    ...
    <div className="min-h-screen w-full bg-white relative flex flex-col">
      ...
      <div className="w-full px-6 pb-8 fixed bottom-0 bg-white">
        ...
        <button
          ...
          className="w-full mt-3 bg-[#FFD60A] text-black py-3 font-bold rounded-xl border border-gray-300"
        >
          Continue as Guest
        </button>
      </div>
